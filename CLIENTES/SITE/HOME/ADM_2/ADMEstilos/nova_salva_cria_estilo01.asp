...
caminho = Server.MapPath("../../clientes/" & cliente & "/estilos/" & arquivo & ".css")
if not fso.fileExists(caminho) then fso.CreateTextFile(caminho)

Set arquivo_css = fso.OpenTExtFile(caminho)
...
'css = "body{background-color: #ffffff;} .sobre{: 1, solid}"
'pattern = "\.*\w+\{(\w*)|(\W*)|(\n*)\})"
pattern = "(\.*\w+\{)[\W\w]*\}"
'pattern = pattern & "\w*"
'pattern = pattern & "\}"
...
function preenche(css, tag, propriedade)
	posicao_tag = instr(css, tag)
	If posicao_tag > 0 then
		css = mid(css, posicao_tag, len(css))
		posicao_tag_fim = instr(css, "}")
		Response.Write(posicao_tag & " - " & posicao_tag_fim & "<br>")
			valor = valor & mid(css, 1, posicao_tag_fim)
		REsponse.Write(valor & "<hr>")
	End if
End function

call preenche(css, "body", "background-color")
On error resume next

'Response.Write(stripIt("body", css))
%><html>
<head>
<script>
var cliente = "<%= cliente %>"
</script>
<title></title><%= replace(css, vbcrlf,"<br>") %>
	  ...
	  <td bordercolor="#CCCCCC"><font size="2" face="Verdana, Arial, Helvetica, sans-serif"><strong><br>
		...
		<tr valign="top" id="cor_fundo"> 
		 <td><font size="2" face="Verdana, Arial, Helvetica, sans-serif"><strong>Cor 
		  ...
		  <input name="body_background-color" id="body_background-color" size="8" onChange="muda_tabela('tabela_body_background-color', this.value)" value="<%= variavel("body_background-color") %>"></td>
		</tr>
		<tr> 
